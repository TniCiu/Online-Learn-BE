...
export const toggleSavePost = async (postId, email) => {
    try {
        return await userInstance.put(`/posts/${postId}/favorite`);
    } catch (error) {
        return Promise.reject(error);
    }
};

export const getStatusFollowed = async (email) => {
    try {
        const res = await userInstance.get(`/${email}/followed`);
        return res.content;
    } catch (error) {
        return Promise.reject(error);
    }
...
export const followUser = async (email) => {
    try {
        await userInstance.put(`/${email}/follow`);
    } catch (error) {
        return Promise.reject(error);
    }
};

export const unFollowUser = async (email) => {
    try {
        await userInstance.put(`/${email}/un-follow`);
    } catch (error) {
        return Promise.reject(error);
    }
};
