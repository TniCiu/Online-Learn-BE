...
import {
    followUser,
    getStatusFollowed,
} from "../../api/apiService/userService";
...
function Profile() {
    ...
    useEffect(() => {
        ...
        if (user.email) {
            const checkIsFollowed = async () => {
                try {
                    const res = await getStatusFollowed(email);
                    setIsFollowed(res);
                } catch (error) {
                    console.log(error);
                }
    ...
    const handleToggleFollow = () => {
        if (isFollowed) {
            toast.promise(followUser(email), {
                loading: "Loading...",
                success: (data) => {
                    setIsFollowed(false);
                    return "Unfollowed";
                                    ...
                                    <div className="flex gap-10 mr-10 text-sm">
                                        {user?.email !== email && (
                                            <button
                                                ...
                                                className={clsx(
                                                    "flex gap-1.5 text-white text-sm font-semibold py-1 items-center hover:opacity-80 transition-all relative rounded-lg px-3",
                                                    {
                                                        "bg-blue-500":
                                                            isFollowed,
                                                        "bg-black": !isFollowed,
